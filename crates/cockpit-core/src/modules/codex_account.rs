...
pub type CodexBatchImportEventEmitter =
    Arc<dyn Fn(&'static str, serde_json::Value) + Send + Sync + 'static>;
...
fn tauri_codex_batch_import_emitter(app: tauri::AppHandle) -> CodexBatchImportEventEmitter {
...
fn emit_codex_batch_import_event<T: serde::Serialize>(
    emitter: &CodexBatchImportEventEmitter,
    event: &'static str,
    payload: T,
) {
...
fn emit_codex_batch_import_progress(
    emitter: &CodexBatchImportEventEmitter,
    payload: CodexBatchImportProgress,
) {
    emit_codex_batch_import_event(emitter, "codex:batch-import-progress", payload);
}

fn emit_codex_batch_import_completed(
    emitter: &CodexBatchImportEventEmitter,
    payload: CodexBatchImportPreview,
) {
    emit_codex_batch_import_event(emitter, "codex:batch-import-completed", payload);
}

fn emit_codex_batch_import_preview(
    emitter: &CodexBatchImportEventEmitter,
    payload: CodexBatchImportPreview,
) {
    emit_codex_batch_import_event(emitter, "codex:batch-import-preview", payload);
}
...
async fn run_codex_batch_import_scan(
    ...
    run_codex_batch_import_resume(emitter, session_id).await;
}

async fn run_codex_batch_import_resume(emitter: CodexBatchImportEventEmitter, session_id: String) {
    ...
    for (index, source_item) in source_items.into_iter().enumerate().skip(start_index) {
        ...
        if let Some(preview) = preview {
            emit_codex_batch_import_preview(&emitter, preview);
        }
    }
    ...
    emit_codex_batch_import_completed(&emitter, preview);
}

fn create_codex_batch_import_session(
    file_paths: &[String],
    check_quota: bool,
...
pub fn start_codex_batch_import_from_files_with_emitter(
    handle: tokio::runtime::Handle,
    file_paths: Vec<String>,
    check_quota: bool,
    emitter: CodexBatchImportEventEmitter,
) -> Result<CodexBatchImportStartResult, String> {
    let session_id = create_codex_batch_import_session(&file_paths, check_quota)?;
    let task_session_id = session_id.clone();
    handle.spawn(async move {
        run_codex_batch_import_scan(emitter, task_session_id, file_paths, check_quota).await;
    });
...
pub fn start_codex_batch_import_from_files(
    ...
    let emitter = tauri_codex_batch_import_emitter(app);
    let session_id = create_codex_batch_import_session(&file_paths, check_quota)?;
    let task_session_id = session_id.clone();
...
fn prepare_codex_batch_import_resume(session_id: &str) -> Result<Option<String>, String> {
    {
        let mut sessions = CODEX_BATCH_IMPORT_SESSIONS.lock().unwrap();
        let session = sessions
            .get_mut(session_id)
...
pub fn resume_codex_batch_import_with_emitter(
    handle: tokio::runtime::Handle,
    session_id: &str,
    emitter: CodexBatchImportEventEmitter,
) -> Result<(), String> {
    if let Some(task_session_id) = prepare_codex_batch_import_resume(session_id)? {
        handle.spawn(async move {
            run_codex_batch_import_resume(emitter, task_session_id).await;
        });
    }
    Ok(())
}
