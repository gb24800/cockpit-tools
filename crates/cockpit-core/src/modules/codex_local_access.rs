...
fn sidecar_binary_file_names() -> Vec<String> {
    let target = std::env::var("COCKPIT_RUST_TARGET")
        .ok()
        .or_else(|| option_env!("COCKPIT_RUST_TARGET").map(str::to_string))
        .unwrap_or_else(default_rust_target);
...
fn build_chat_test_stream_event_payload(session_id: &str, payload: Value) -> Value {
...
fn emit_chat_test_stream_event_with<F>(emit_event: &mut F, session_id: &str, payload: Value)
where
    F: FnMut(Value),
{
    emit_event(build_chat_test_stream_event_payload(session_id, payload));
}

fn emit_chat_test_stream_failure_with<F>(
    emit_event: &mut F,
    session_id: &str,
    ...
    F: FnMut(Value),
{
    emit_chat_test_stream_event_with(
        emit_event,
        session_id,
        json!({
            "type": "error",
...
async fn run_local_access_chat_stream_dialog_with_events<F>(
    emit_event: &mut F,
    session_id: &str,
    base_url: &str,
    api_key: &str,
    ...
    F: FnMut(Value),
{
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        while let Some(index) = buffer.find("\n\n") {
            ...
            if handle_local_access_chat_stream_frame_with_events(emit_event, session_id, &frame) {
                emit_chat_test_stream_event_with(
                    emit_event,
                    session_id,
                    json!({
    ...
    if !buffer.trim().is_empty()
        && handle_local_access_chat_stream_frame_with_events(emit_event, session_id, &buffer)
    {
        emit_chat_test_stream_event_with(
            emit_event,
            session_id,
...
fn handle_local_access_chat_stream_frame_with_events<F>(
    emit_event: &mut F,
    session_id: &str,
    frame: &str,
) -> bool
...
pub async fn stream_chat_local_access_with_dialog_events<F>(
    session_id: String,
    model_id: String,
    messages: Vec<CodexLocalAccessChatMessage>,
    mut emit_event: F,
    ...
    F: FnMut(Value),
{
    ...
    let Some(collection) = state.collection.clone() else {
        emit_chat_test_stream_failure_with(
            &mut emit_event,
            &session_id,
    ...
    match run_local_access_chat_stream_dialog_with_events(
        &mut emit_event,
        &session_id,
        &base_url,
        &collection.api_key,
